...
class RayPPOTrainer:
    ...
    def _score_chunk_state_answer_source_consistency(
        self,
        state_prompts: DataProto,
        chunk_output: DataProto,
        probe_output: DataProto,
        ...
        if "chunk_state_source_answer" not in state_prompts.non_tensor_batch:
            raise ValueError(
                "chunk_state_score_mode='answer_source_consistency' requires full-rollout source answer metadata"
            )

        source_answers = [str(x) for x in state_prompts.non_tensor_batch["chunk_state_source_answer"]]
        source_answer_mass = np.asarray(
            state_prompts.non_tensor_batch.get(
        ...
        state_positive_counts = [0 for _ in range(len(state_prompts))]
        state_answer_counts = [0 for _ in range(len(state_prompts))]
        state_label_counts = [0 for _ in range(len(state_prompts))]
        for probe_idx in range(len(probe_output)):
            chunk_idx = probe_idx // probe_samples
            state_idx = chunk_idx // candidates
            chunk_len = int(chunk_mask[chunk_idx].sum().item())
            ...
            if answer is not None:
                answer = simplify_expression_string(answer)
            label = source_answers[state_idx]
            has_answer = answer is not None and answer != "None"
            has_label = bool(label) and label != "None"
            if has_answer:
                state_answer_counts[state_idx] += 1
            if has_label:
                state_label_counts[state_idx] += 1
            if not has_answer or not has_label:
                score = 0.0
            elif answer == label:
                score = 1.0
        ...
        scores = score_matrix.reshape(-1)
        label_consistent = (score_matrix > 0.0).to(dtype=torch.float32)
        valid_source = np.asarray([count > 0 for count in state_label_counts], dtype=np.float32)
        answer_coverage_arr = np.asarray(
            [count / max(group_size, 1) for count in state_answer_counts],
            dtype=np.float32,
        )
        source_answer_mass = source_answer_mass * valid_source

        state_prompts.non_tensor_batch["chunk_state_majority_ratio"] = source_answer_mass.astype(np.float32)
        state_prompts.non_tensor_batch["chunk_state_answer_coverage"] = answer_coverage_arr
        state_prompts.non_tensor_batch["chunk_state_label_consistent"] = label_consistent.cpu().numpy().astype(
            np.float32
        )
        metrics = {
            "chunk_state_answer_source_consistency/group_size": float(group_size),
            "chunk_state_answer_source_consistency/source_answer_mass_mean": float(source_answer_mass.mean())
            if len(source_answer_mass)
            else 0.0,
            "chunk_state_answer_source_consistency/source_answer_mass_min": float(source_answer_mass.min())
            if len(source_answer_mass)
            else 0.0,
            "chunk_state_answer_source_consistency/valid_source_ratio": float(valid_source.mean())
            if len(valid_source)
            else 0.0,
            "chunk_state_answer_source_consistency/answer_coverage_mean": float(answer_coverage_arr.mean())
            if len(answer_coverage_arr)
            else 0.0,
            "chunk_state_answer_source_consistency/raw_positive_ratio": raw_probe_scores.mean().item()
            if len(raw_probe_scores)
            else 0.0,
            "chunk_state_answer_source_consistency/label_consistent_ratio": label_consistent.mean().item()
            if len(label_consistent)
            else 0.0,
            "chunk_state_answer_source_consistency/state_positive_ratio": float(
                np.mean([count > 0 for count in state_positive_counts])
            )
            if state_positive_counts
            else 0.0,
    ...
    def _score_chunk_state_answer_value_gain(
        self,
        state_prompts: DataProto,
        chunk_output: DataProto,
        probe_output: DataProto,
        ...
        if "chunk_state_prompt_majority_label" not in state_prompts.non_tensor_batch:
            raise ValueError("chunk_state_score_mode='answer_value_gain' requires prompt-level majority labels")
        ...
        denom = (1.0 - baseline).clamp(min=1e-6)
        gain_matrix = ((hit_rate - baseline) / denom).clamp(min=0.0, max=1.0)
        scores = gain_matrix.reshape(-1)
        label_consistent = (gain_matrix > 0.0).to(dtype=torch.float32)
        answer_coverage_arr = np.asarray(
            [count / max(group_size, 1) for count in state_answer_counts],
            dtype=np.float32,
        )
        baseline_arr = baseline_ratios.astype(np.float32)
        max_gain = gain_matrix.max(dim=-1).values.detach().cpu().numpy().astype(np.float32)
        mean_hit = hit_rate.mean(dim=-1).detach().cpu().numpy().astype(np.float32)
        improved_state = (max_gain > 0.0).astype(np.float32)
        ...
        metrics = {
            "chunk_state_answer_value_gain/group_size": float(group_size),
            "chunk_state_answer_value_gain/baseline_ratio_mean": float(baseline_arr.mean())
            if len(baseline_arr)
            else 0.0,
            "chunk_state_answer_value_gain/baseline_ratio_min": float(baseline_arr.min())
            if len(baseline_arr)
            else 0.0,
            "chunk_state_answer_value_gain/answer_coverage_mean": float(answer_coverage_arr.mean())
            if len(answer_coverage_arr)
            else 0.0,
            "chunk_state_answer_value_gain/hit_rate_mean": float(mean_hit.mean()) if len(mean_hit) else 0.0,
            "chunk_state_answer_value_gain/gain_mean": gain_matrix.mean().item() if len(gain_matrix) else 0.0,
            "chunk_state_answer_value_gain/max_gain_mean": float(max_gain.mean()) if len(max_gain) else 0.0,
            "chunk_state_answer_value_gain/improved_state_ratio": float(improved_state.mean())
            if len(improved_state)
            else 0.0,
            "chunk_state_answer_value_gain/label_consistent_ratio": label_consistent.mean().item()
            if len(label_consistent)
            else 0.0,
        }
        return scores, metrics
    ...
    def _score_chunk_state_answer_support_mass(
        self,
        state_prompts: DataProto,
        chunk_output: DataProto,
        probe_output: DataProto,
        ...
        if "chunk_state_prompt_answer_mass" not in state_prompts.non_tensor_batch:
            raise ValueError("chunk_state_score_mode='answer_support_mass' requires full-rollout answer metadata")

        cfg = self.config.ttrl
        score_type = str(cfg.get("chunk_state_answer_support_score", "mass"))
        min_mass = float(cfg.get("chunk_state_answer_support_min_mass", 0.0))
        prompt_mass_values = [
            json.loads(str(value)) if str(value) else {}
            for value in state_prompts.non_tensor_batch["chunk_state_prompt_answer_mass"]
        ]
        ...
        for probe_idx in range(len(probe_output)):
            ...
            mass = (
                float(prompt_mass_values[state_idx].get(answer_text, 0.0))
        ...
        mass_matrix = mass_tensor.mean(dim=-1)
        source_mass = source_answer_mass.view(len(state_prompts), 1)
        if score_type == "mass":
            score_matrix = mass_matrix
        elif score_type == "gain":
            score_matrix = (mass_matrix - source_mass).clamp(min=0.0)
        elif score_type == "relative_gain":
            score_matrix = ((mass_matrix - source_mass) / (1.0 - source_mass).clamp(min=1e-6)).clamp(
                min=0.0,
                max=1.0,
            )
        else:
            raise ValueError(f"Unsupported ttrl.chunk_state_answer_support_score={score_type!r}")

        scores = score_matrix.reshape(-1)
        label_consistent = (score_matrix > 0.0).to(dtype=torch.float32)
        answer_coverage_arr = valid_tensor.mean(dim=(1, 2)).detach().cpu().numpy().astype(np.float32)
        source_mass_arr = source_answer_mass.detach().cpu().numpy().astype(np.float32)
        max_mass = mass_matrix.max(dim=-1).values.detach().cpu().numpy().astype(np.float32)
        mean_mass = mass_matrix.mean(dim=-1).detach().cpu().numpy().astype(np.float32)
        improved_state = (score_matrix.max(dim=-1).values > 0.0).float().detach().cpu().numpy().astype(np.float32)

        state_prompts.non_tensor_batch["chunk_state_majority_ratio"] = source_mass_arr
        state_prompts.non_tensor_batch["chunk_state_answer_coverage"] = answer_coverage_arr
        state_prompts.non_tensor_batch["chunk_state_label_consistent"] = label_consistent.cpu().numpy().astype(
            np.float32
        )
        metrics = {
            "chunk_state_answer_support_mass/group_size": float(candidates * probe_samples),
            "chunk_state_answer_support_mass/score_type_mass": float(score_type == "mass"),
            "chunk_state_answer_support_mass/score_type_gain": float(score_type == "gain"),
            "chunk_state_answer_support_mass/score_type_relative_gain": float(score_type == "relative_gain"),
            "chunk_state_answer_support_mass/min_mass": min_mass,
            "chunk_state_answer_support_mass/source_mass_mean": float(source_mass_arr.mean())
            if len(source_mass_arr)
            else 0.0,
            "chunk_state_answer_support_mass/probe_support_coverage_mean": float(answer_coverage_arr.mean())
            if len(answer_coverage_arr)
            else 0.0,
            "chunk_state_answer_support_mass/mass_mean": float(mean_mass.mean()) if len(mean_mass) else 0.0,
            "chunk_state_answer_support_mass/max_mass_mean": float(max_mass.mean()) if len(max_mass) else 0.0,
            "chunk_state_answer_support_mass/score_mean": score_matrix.mean().item()
            if len(score_matrix)
            else 0.0,
            "chunk_state_answer_support_mass/label_consistent_ratio": label_consistent.mean().item()
            if len(label_consistent)
            else 0.0,
            "chunk_state_answer_support_mass/improved_state_ratio": float(improved_state.mean())
            if len(improved_state)
            else 0.0,
        }
        return scores, metrics

    def _score_chunk_state_suffix_support(
        self,
        full_batch: DataProto,
        state_prompts: DataProto,
        chunk_output: DataProto,
        ...
        if len(chunk_output) != len(state_prompts) * candidates:
            raise ValueError(
                f"Expected {len(state_prompts) * candidates} suffix outputs, got {len(chunk_output)}"
            )
        if "chunk_state_prompt_answer_mass" not in state_prompts.non_tensor_batch:
            raise ValueError("chunk_state_score_mode='suffix_support' requires full-rollout answer metadata")

        cfg = self.config.ttrl
        score_type = str(cfg.get("chunk_state_suffix_support_score_type", "gain"))
        min_mass = float(cfg.get("chunk_state_suffix_support_min_mass", 0.0))
        ...
        prompt_top_mass = torch.as_tensor(
            np.asarray(
                state_prompts.non_tensor_batch.get(
                    "chunk_state_source_prompt_top_mass",
                    np.zeros(len(state_prompts), dtype=np.float32),
        ...
        suffix_mask = chunk_output.batch["response_mask"].bool()

        mass_values = []
        valid_values = []
        boxed_counts = []
        repeated_boxed_values = []
        empty_answer_values = []
        answer_values = []
        suffix_lengths = []
        completion_lengths = []
        for suffix_idx in range(len(chunk_output)):
            state_idx = suffix_idx // candidates
            source_idx = int(source_indices[state_idx])
            boundary = int(boundaries[state_idx])
            source_response_len = int(full_response_mask[source_idx].sum().item())
            prefix_len = max(0, min(boundary, source_response_len))
            suffix_len = int(suffix_mask[suffix_idx].sum().item())
            prefix_ids = full_batch.batch["responses"][source_idx, :prefix_len]
            suffix_ids = chunk_output.batch["responses"][suffix_idx, :suffix_len]
            ...
            mass_values.append(mass)
            valid_values.append(1.0 if mass > 0.0 else 0.0)
            boxed_counts.append(float(boxed_count))
            repeated_boxed_values.append(
                1.0
                if boxed_count > int(cfg.get("chunk_state_full_rollout_guard_max_boxed_count", 4))
                else 0.0
            )
            empty_answer_values.append(1.0 if not answer_text or answer_text == "None" else 0.0)
            answer_values.append(answer_text if answer_text else "None")
            suffix_lengths.append(float(suffix_len))
        ...
        if score_type == "mass":
            ...
            score_matrix = (mass_matrix - baseline + gain_slack).clamp(min=0.0, max=1.0)
        elif score_type == "relative_gain":
            score_matrix = ((mass_matrix - baseline + gain_slack) / (1.0 - baseline).clamp(min=1e-6)).clamp(
                min=0.0,
                max=1.0,
            )
        else:
            state_baseline = mass_matrix.mean(dim=-1, keepdim=True)
            score_matrix = (mass_matrix - state_baseline + gain_slack).clamp(min=0.0, max=1.0)

        positive_margin = (mass_matrix - baseline).max(dim=-1).values
        keep_state = positive_margin >= min_positive_margin
        score_matrix = score_matrix * keep_state.view(-1, 1).to(dtype=score_matrix.dtype)
        label_consistent = (score_matrix > 0.0).to(dtype=torch.float32)
        support_coverage = valid_matrix.mean(dim=-1)
        state_mean_mass = mass_matrix.mean(dim=-1)
        state_max_mass = mass_matrix.max(dim=-1).values
        ...
        state_prompts.non_tensor_batch["chunk_state_majority_ratio"] = prompt_top_mass.cpu().numpy().astype(
            np.float32
        )
        state_prompts.non_tensor_batch["chunk_state_answer_coverage"] = support_coverage.cpu().numpy().astype(
            np.float32
        )
        state_prompts.non_tensor_batch["chunk_state_positive_margin"] = positive_margin.cpu().numpy().astype(
            np.float32
        )
        state_prompts.non_tensor_batch["chunk_state_future_support_keep"] = keep_state.float().cpu().numpy().astype(
            np.float32
        )
        state_prompts.non_tensor_batch["chunk_state_future_support_soft_weight"] = (
            future_support_soft_weight.cpu().numpy().astype(np.float32)
        )
        state_prompts.non_tensor_batch["chunk_state_future_support_state_mean_mass"] = (
            state_mean_mass.cpu().numpy().astype(np.float32)
        )
        state_prompts.non_tensor_batch["chunk_state_future_support_state_max_mass"] = (
            state_max_mass.cpu().numpy().astype(np.float32)
        )
        state_prompts.non_tensor_batch["chunk_state_future_support_state_top_margin"] = (
            state_top_margin.cpu().numpy().astype(np.float32)
        )
        scores = score_matrix.reshape(-1)
        metrics = {
            "chunk_state_suffix_support/group_size": float(candidates),
            "chunk_state_suffix_support/score_type_mass": float(score_type == "mass"),
            "chunk_state_suffix_support/score_type_gain": float(score_type == "gain"),
            "chunk_state_suffix_support/score_type_relative_gain": float(score_type == "relative_gain"),
            "chunk_state_suffix_support/score_type_posterior_gain": float(score_type == "posterior_gain"),
            "chunk_state_suffix_support/min_mass": min_mass,
            ...
            "chunk_state_suffix_support/prompt_top_mass_mean": prompt_top_mass.mean().item()
            if len(prompt_top_mass)
            else 0.0,
            "chunk_state_suffix_support/support_coverage_mean": support_coverage.mean().item()
            ...
            "chunk_state_suffix_support/score_max_mean": score_matrix.max(dim=-1).values.mean().item()
            if len(score_matrix)
            else 0.0,
            "chunk_state_suffix_support/positive_margin_mean": positive_margin.mean().item()
            if len(positive_margin)
            else 0.0,
            "chunk_state_suffix_support/state_top_margin_mean": state_top_margin.mean().item()
            if len(state_top_margin)
            else 0.0,
            "chunk_state_suffix_support/state_keep_ratio": keep_state.float().mean().item()
            if len(keep_state)
            else 0.0,
            "chunk_state_suffix_support/label_consistent_ratio": label_consistent.mean().item()
            if len(label_consistent)
            else 0.0,
            ...
            "chunk_state_suffix_support/repeated_boxed_ratio": float(np.mean(repeated_boxed_values))
            if repeated_boxed_values
            else 0.0,
            "chunk_state_suffix_support/boxed_count_mean": float(np.mean(boxed_counts)) if boxed_counts else 0.0,
            "chunk_state_suffix_support/suffix_len_mean": float(np.mean(suffix_lengths)) if suffix_lengths else 0.0,
            "chunk_state_suffix_support/completion_len_mean": float(np.mean(completion_lengths))
            if completion_lengths
            else 0.0,
        }
        return scores, metrics

    def _score_chunk_state_future_support_gain(
        self,
        state_prompts: DataProto,
        chunk_output: DataProto,
        probe_output: DataProto,
        ...
        if "chunk_state_prompt_answer_mass" not in state_prompts.non_tensor_batch:
            raise ValueError("chunk_state_score_mode='future_support_gain' requires full-rollout answer metadata")

        cfg = self.config.ttrl
        min_mass = float(cfg.get("chunk_state_future_support_min_mass", 0.0))
        max_mass_coef = float(cfg.get("chunk_state_future_support_max_mass_coef", 0.25))
        score_type = str(cfg.get("chunk_state_future_support_score_type", "slack_gain"))
        ...
        smoothed_support_expected_value_rows = []
        smoothed_support_overlap_rows = []
        ...
        for state_idx, support in enumerate(prompt_mass_values):
            ...
            source_oov_tv = source_tv
            source_oov_tv_values.append(source_oov_tv)
            row = []
            oov_row = []
            affinity_row = []
            smoothed_oov_row = []
            ...
            expected_value_row = []
            overlap_row = []
            smoothed_expected_value_row = []
            smoothed_overlap_row = []
            entropy_row = []
            for cand_idx in range(candidates):
                ...
                if smooth_total > 0.0:
                    ...
                    smoothed_expected_value = 0.0
                    smoothed_overlap = 0.0
                    for answer in support_dist.keys():
                        smoothed_prob = (
                            counts.get(answer, 0) + max(support_prior_smoothing, 0.0) * support_dist.get(answer, 0.0)
                        ) / smooth_total
                        support_prob = support_dist.get(answer, 0.0)
                        smoothed_support_parts.append(abs(smoothed_prob - support_prob))
                        smoothed_support_mass += smoothed_prob
                        smoothed_expected_value += smoothed_prob * support_prob
                        smoothed_overlap += min(smoothed_prob, support_prob)
                    ...
                    smoothed_expected_value = expected_value
                    smoothed_overlap = overlap
                row.append(float(tv))
                ...
                smoothed_expected_value_row.append(float(smoothed_expected_value))
                smoothed_overlap_row.append(float(smoothed_overlap))
            ...
            smoothed_support_expected_value_rows.append(smoothed_expected_value_row)
            smoothed_support_overlap_rows.append(smoothed_overlap_row)
        ...
        smoothed_support_expected_value = torch.tensor(smoothed_support_expected_value_rows, dtype=torch.float32)
        smoothed_support_overlap = torch.tensor(smoothed_support_overlap_rows, dtype=torch.float32)
        ...
        elif score_type == "transport_affinity":
        ...
        elif score_type == "posterior_support_match":
            score_matrix = (
                (0.5 * smoothed_support_expected_value + 0.5 * smoothed_support_overlap)
        ...
        smoothed_support_expected_value_arr = (
            smoothed_support_expected_value.mean(dim=-1).detach().cpu().numpy().astype(np.float32)
        )
        smoothed_support_overlap_arr = smoothed_support_overlap.mean(dim=-1).detach().cpu().numpy().astype(np.float32)
        ...
        metrics = {
            "chunk_state_future_support_gain/group_size": float(candidates * probe_samples),
            "chunk_state_future_support_gain/min_mass": min_mass,
            "chunk_state_future_support_gain/max_mass_coef": max_mass_coef,
            "chunk_state_future_support_gain/score_type_slack_gain": float(score_type == "slack_gain"),
            "chunk_state_future_support_gain/score_type_positive_gain": float(score_type == "positive_gain"),
            ...
            "chunk_state_future_support_gain/score_type_transport_affinity": float(
            ...
            "chunk_state_future_support_gain/prompt_top_mass_mean": float(prompt_top_mass_arr.mean())
            if len(prompt_top_mass_arr)
            else 0.0,
            ...
            "chunk_state_future_support_gain/source_oov_tv_mean": float(source_oov_tv_arr.mean())
            if len(source_oov_tv_arr)
            else 0.0,
            "chunk_state_future_support_gain/candidate_oov_tv_mean": float(candidate_oov_tv_arr.mean())
            if len(candidate_oov_tv_arr)
            else 0.0,
            ...
            "chunk_state_future_support_gain/smoothed_support_expected_value_mean": float(
                smoothed_support_expected_value_arr.mean()
            )
            if len(smoothed_support_expected_value_arr)
            else 0.0,
            "chunk_state_future_support_gain/smoothed_support_overlap_mean": float(
                smoothed_support_overlap_arr.mean()
            )
            if len(smoothed_support_overlap_arr)
            else 0.0,
            ...
            "chunk_state_future_support_gain/improved_state_ratio": float(improved_state.mean())
            if len(improved_state)
            else 0.0,
    ...
    def _score_chunk_state_answer_distribution(
        ...
        for state_idx in range(len(state_prompts)):
            ...
            state_answers = decoded_answers[start:end]
            valid_answers = [answer for answer in state_answers if answer != "None"]
            valid_count = len(valid_answers)
            answer_coverage.append(valid_count / max(group_size, 1))
            counts = Counter(valid_answers)
            unique_answer_counts.append(len(counts))
            if counts:
                ordered_counts = sorted(counts.values(), reverse=True)
                top_mass = ordered_counts[0] / max(valid_count, 1)
                second_mass = ordered_counts[1] / max(valid_count, 1) if len(ordered_counts) > 1 else 0.0
                top_masses.append(top_mass)
                top2_margins.append(top_mass - second_mass)
            else:
                top_masses.append(0.0)
                top2_margins.append(0.0)

            for answer in state_answers:
                if answer == "None" or valid_count <= 0:
                    score_values.append(0.0)
                else:
                    # State-local label estimation: answers supported by more completions
                    # define a softer search-improved target than a single hard majority.
                    score_values.append(counts[answer] / valid_count)
            state_positive.append(any(value > 0.0 for value in score_values[-group_size:]))

        raw_scores = torch.tensor(score_values, dtype=torch.float32)
        score_matrix = raw_scores.view(len(state_prompts), candidates, probe_samples).mean(dim=-1)
        scores = score_matrix.reshape(-1)
        label_consistent = (score_matrix > 0.0).to(dtype=torch.float32)
        answer_coverage_arr = np.asarray(answer_coverage, dtype=np.float32)
        top_masses_arr = np.asarray(top_masses, dtype=np.float32)
        unique_answer_arr = np.asarray(unique_answer_counts, dtype=np.float32)
        top2_margin_arr = np.asarray(top2_margins, dtype=np.float32)

        state_prompts.non_tensor_batch["chunk_state_majority_ratio"] = top_masses_arr
        state_prompts.non_tensor_batch["chunk_state_answer_coverage"] = answer_coverage_arr
        state_prompts.non_tensor_batch["chunk_state_label_consistent"] = label_consistent.cpu().numpy().astype(
            np.float32
        )
        metrics = {
            "chunk_state_answer_distribution/group_size": float(group_size),
            "chunk_state_answer_distribution/top_mass_mean": float(top_masses_arr.mean())
            if len(top_masses_arr)
            else 0.0,
            "chunk_state_answer_distribution/top_mass_min": float(top_masses_arr.min())
            if len(top_masses_arr)
            else 0.0,
            "chunk_state_answer_distribution/top2_margin_mean": float(top2_margin_arr.mean())
            if len(top2_margin_arr)
            else 0.0,
            "chunk_state_answer_distribution/unique_answer_mean": float(unique_answer_arr.mean())
            if len(unique_answer_arr)
            else 0.0,
            "chunk_state_answer_distribution/answer_coverage_mean": float(answer_coverage_arr.mean())
            if len(answer_coverage_arr)
            else 0.0,
            "chunk_state_answer_distribution/raw_score_mean": raw_scores.mean().item()
            if len(raw_scores)
            else 0.0,
            "chunk_state_answer_distribution/label_consistent_ratio": label_consistent.mean().item()
            if len(label_consistent)
            else 0.0,
            "chunk_state_answer_distribution/state_positive_ratio": float(np.mean(state_positive))
            if state_positive
            else 0.0,
        }
        return scores, metrics
    ...
    def _score_chunk_state_support_flow(
        self,
        state_prompts: DataProto,
        candidates: int,
    ) -> tuple[torch.Tensor, dict]:
        if "chunk_state_support_anchor_scores" not in state_prompts.non_tensor_batch:
            raise ValueError("chunk_state_score_mode='support_flow' requires injected support anchor scores")

        cfg = self.config.ttrl
        score_type = str(cfg.get("chunk_state_support_flow_score_type", "gain"))
        gain_slack = float(cfg.get("chunk_state_support_flow_gain_slack", 0.0))
        ...
        if score_type in {"mass", "soft_mass"}:
            ...
            score_matrix = ((anchor_mass - baseline + gain_slack) / (1.0 - baseline).clamp(min=1e-6)).clamp(
                min=0.0,
                max=1.0,
            )
        ...
        state_prompts.non_tensor_batch["chunk_state_label_consistent"] = label_consistent.numpy().astype(np.float32)
        state_prompts.non_tensor_batch["chunk_state_majority_ratio"] = prompt_top_mass.astype(np.float32)
        ...
        if source_prior_weight > 0.0:
            prior = torch.ones_like(score_matrix)
            injected = torch.as_tensor(
                np.asarray(
                    state_prompts.non_tensor_batch.get(
                        "chunk_state_support_anchor_injected",
                        np.zeros((len(state_prompts), candidates), dtype=np.float32),
            ...
            prior = torch.where(injected > 0.0, torch.full_like(prior, source_prior_weight), prior)
            state_prompts.non_tensor_batch["chunk_state_target_prior"] = prior.numpy().astype(np.float32)
        ...
        metrics = {
            ...
            "chunk_state_support_flow/state_keep_ratio": keep_state.float().mean().item() if len(keep_state) else 0.0,
            "chunk_state_support_flow/score_mean": score_matrix.mean().item() if len(score_matrix) else 0.0,
            "chunk_state_support_flow/score_max_mean": state_max.mean().item() if len(state_max) else 0.0,
            "chunk_state_support_flow/label_consistent_ratio": label_consistent.mean().item()
            if len(label_consistent)
            else 0.0,
        }
        return scores, metrics
    ...
    def _build_chunk_actor_batch(
        ...
        zero_inconsistent_candidates = bool(cfg.get("chunk_state_zero_inconsistent_candidates", False))
        prune_zero_weight_samples = bool(cfg.get("chunk_state_prune_zero_weight_samples", False))
        ...
        if label_consistent_only:
            ...
            label_consistent = (score_matrix > 0.0).float()
        if zero_inconsistent_candidates:
            raw_weights = torch.where(
                label_consistent > 0.0,
                torch.pow(score_matrix + eps, alpha),
                torch.zeros_like(score_matrix),
            )
        else:
            raw_weights = torch.pow(score_matrix + eps, alpha)
        target_prior = torch.as_tensor(
            state_prompts.non_tensor_batch.get(
        ...
        if target_prior.shape != raw_weights.shape:
            target_prior = target_prior.reshape(raw_weights.shape)
        target_prior = target_prior.clamp(min=0.0)
        raw_weights = raw_weights * target_prior
        ...
        if prune_zero_weight_samples and len(powerflow_flat_weights) > 0:
            ...
            if len(nonzero_indices) > 0:
                ...
                flat_boxed_rewards = flat_boxed_rewards[nonzero_indices]
                flat_weights = flat_weights[nonzero_indices]
                flat_loss_weights = flat_loss_weights[nonzero_indices]
                powerflow_flat_weights = powerflow_flat_weights[nonzero_indices]
        ...
        actor_batch["boxed_reward"][torch.arange(len(responses)), response_lengths - 1] = flat_boxed_rewards
        ...
        metrics = {
            ...
            "chunk_state/target_prior_mean": target_prior.mean().detach().item(),
    ...
    def _run_chunk_state_training_step(self, full_batch: DataProto, metrics: dict, timing_raw: dict):
        ...
        with marked_timer("chunk_state_chunks", timing_raw, color="red"):
            ...
            chunk_max_tokens = (
                int(cfg.get("chunk_state_suffix_max_tokens", self.config.data.max_response_length))
                if score_mode == "suffix_support"
                else chunk_size
            )
            chunk_prompts.meta_info["kwargs"] = {"n": candidates, "max_tokens": chunk_max_tokens}
        ...
        if score_mode in {"support_anchor", "support_flow", "suffix_support"}:
        ...
        with marked_timer("chunk_state_score", timing_raw, color="yellow"):
            ...
            if score_mode == "probe_reward":
            ...
            elif score_mode == "answer_distribution":
                scores, distribution_metrics = self._score_chunk_state_answer_distribution(
                    state_prompts=state_prompts,
                    chunk_output=chunk_output,
                    probe_output=probe_output,
                    candidates=candidates,
                ...
                metrics["chunk_state_probe/raw_positive_ratio"] = distribution_metrics[
                    "chunk_state_answer_distribution/raw_score_mean"
                ]
            elif score_mode == "answer_value_gain":
                scores, value_gain_metrics = self._score_chunk_state_answer_value_gain(
                    state_prompts=state_prompts,
                    chunk_output=chunk_output,
                    probe_output=probe_output,
                    candidates=candidates,
            ...
            elif score_mode == "answer_support_mass":
                scores, support_mass_metrics = self._score_chunk_state_answer_support_mass(
                    state_prompts=state_prompts,
                    chunk_output=chunk_output,
                    probe_output=probe_output,
                    candidates=candidates,
                ...
                metrics["chunk_state_probe/raw_positive_ratio"] = support_mass_metrics[
                    "chunk_state_answer_support_mass/score_mean"
                ]
            elif score_mode == "future_support_gain":
                scores, future_support_metrics = self._score_chunk_state_future_support_gain(
                    state_prompts=state_prompts,
                    chunk_output=chunk_output,
                    probe_output=probe_output,
                    candidates=candidates,
            ...
            elif score_mode == "support_flow":
                scores, support_flow_metrics = self._score_chunk_state_support_flow(
                    state_prompts=state_prompts,
                    candidates=candidates,
                )
                metrics.update(support_flow_metrics)
                metrics["chunk_state_support_flow/raw_positive_ratio"] = support_flow_metrics[
                    "chunk_state_support_flow/score_mean"
                ]
            elif score_mode == "suffix_support":
                scores, suffix_support_metrics = self._score_chunk_state_suffix_support(
                    full_batch=full_batch,
                    state_prompts=state_prompts,
                    chunk_output=chunk_output,
                    candidates=candidates,
                )
                metrics.update(suffix_support_metrics)
                metrics["chunk_state_suffix_support/raw_positive_ratio"] = suffix_support_metrics[
                    "chunk_state_suffix_support/score_mean"
                ]
                ...
                metrics["chunk_state_probe/raw_positive_ratio"] = source_consistency_metrics[
                    "chunk_state_answer_source_consistency/raw_positive_ratio"
                ]
            else:
            ...
            metrics["chunk_state_score/mode_support_flow"] = float(score_mode == "support_flow")
            metrics["chunk_state_score/mode_suffix_support"] = float(score_mode == "suffix_support")
            metrics["chunk_state_score/mode_answer_source_consistency"] = float(
                score_mode == "answer_source_consistency"
            )
            if score_mode not in {"support_anchor", "support_flow", "suffix_support"}:
