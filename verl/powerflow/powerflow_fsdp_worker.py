...
"""PowerFlow FSDP Worker that uses PowerFlowActor instead of standard DPActor."""
...
from powerflow.powerflow_actor import PowerFlowActor, ProjZModule
...
try:
    ...
    def get_vl_model_vision_tower(_model):
        return None
...
class PowerFlowActorRolloutRefWorker(ActorRolloutRefWorker):
    """
    PowerFlow version of ActorRolloutRefWorker that uses PowerFlowActor.

    This worker adds PowerFlow-specific modifications:
    - ProjZModule for log Z estimation (added in _build_model_optimizer)
    - PowerFlowActor with trajectory balance loss (replaces standard DPActor)
    """
    ...
    def _build_model_optimizer(
        ...
        attn_implementation = override_model_config.get(
            "attn_implementation", os.environ.get("POWERFLOW_ATTN_IMPL", "sdpa")
        )
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # ==== PowerFlow: inject ProjZ BEFORE FSDP wrap ====
            if role == "actor" and self._is_actor:
                n_dim = actor_module.config.hidden_size
                proj_layers = getattr(self.config.actor, "proj_layer", 3)
                # Get init_offset from config, default: 0.36 * 3.0 = 1.08 (init_ref_log_prob * (beta_coef - 1))
                init_ref_log_prob = getattr(self.config.actor, "init_ref_log_prob", 0.36)
                beta_coef = getattr(self.config.actor, "beta_coef", 4.0)
                init_offset = init_ref_log_prob * (beta_coef - 1)
                actor_module.add_module("proj_z", ProjZModule(n_dim, num_layers=proj_layers, init_offset=init_offset))

                if self.rank == 0:
                    print(f"[PowerFlow] Added proj_z (layers={proj_layers}, hidden={n_dim}, init_offset={init_offset}) BEFORE FSDP wrap")
            # ===============================================
        ...
        # ==================== FIX START ====================
        # [PowerFlow Fix] 强制包裹 ProjZModule
        # 原因：如果不单独包裹，proj_z 会归属 Root FSDP。
        # 直接调用 model.proj_z() 会绕过 Root 的 forward hook，导致访问到空的 storage。
        # 单独包裹后，model.proj_z() 会触发其自身的 FSDP hook，正确加载参数。
        
        if role == "actor" and self._is_actor:
            original_policy = auto_wrap_policy
            
            def powerflow_wrap_policy(module, recurse, nonwrapped_numel):
                # 如果当前模块是 ProjZModule，强制包裹（返回 True）
                if isinstance(module, ProjZModule):
                    return True
                # 否则使用原有的策略
                if original_policy:
                    return original_policy(module, recurse, nonwrapped_numel)
                return False
            
            auto_wrap_policy = powerflow_wrap_policy
            
            if self.rank == 0:
                print(f"[PowerFlow] Updated auto_wrap_policy to explicitly wrap ProjZModule")
        # ==================== FIX END ====================
    ...
    def init_model(self):
        """Override init_model to use PowerFlowActor instead of DataParallelPPOActor."""
        # Call parent's init_model to set up the FSDP model (with proj_z already added)
        super().init_model()

        if self._is_actor:
            if self.config.actor.get("powerflow_use_ttrl_actor", False):
                if self.rank == 0:
                    print("[PowerFlow] Using TTRL DataParallelPPOActor PowerFlow path")
                return

            # Replace the actor with the vendored original PowerFlowActor.
            if self.rank == 0:
                print("[PowerFlow] Replacing DataParallelPPOActor with PowerFlowActor")
            self.actor = PowerFlowActor(
    ...
    async def rollout_mode(self):
        ...
        PowerFlow's proj_z module is only needed during training for estimating log Z.
        ...
        # ==== PowerFlow: Filter out proj_z parameters ====
        ...
        if num_proj_z_filtered > 0 and self.rank == 0:
            print(f"[PowerFlow] Filtered {num_proj_z_filtered} proj_z parameters before syncing to vLLM")
        # ===============================================
