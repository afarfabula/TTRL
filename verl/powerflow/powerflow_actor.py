...
def _split_dataproto_by_size(data: DataProto, batch_size: int):
    assert len(data) % batch_size == 0, f"{len(data)} must be divisible by {batch_size}"
    return data.chunk(len(data) // batch_size)

def masked_std(values, mask, axis):
    """Compute standard deviation of tensor with masked values."""
    mean = verl_F.masked_mean(values, mask, axis=axis)
    total_mean = verl_F.masked_mean(values, mask)
    return ((mean - total_mean)**2).mean(axis=axis)**.5

class ProjZModule(torch.nn.Module):
    """Projection network for estimating log partition function Z in PowerFlow."""

    def __init__(self, hidden_size: int, num_layers: int = 3, dropout: float = 0.1, init_offset: float = 1.08):
        """
        Initialize ProjZModule.

        Args:
            hidden_size: Hidden dimension size of the model.
            num_layers: Number of layers in the projection network.
            dropout: Dropout rate.
            init_offset: Initial offset value, typically init_ref_log_prob * (beta_coef - 1).
                         Default: 0.36 * 3.0 = 1.08 (for init_ref_log_prob=0.36, beta_coef=4.0).
        """
        super().__init__()
        layers = []
    ...
    def forward(self, x):
        """Forward pass with init_offset applied."""
        return self.net(x) - self.init_offset


class PowerFlowActor(DataParallelPPOActor):
    """PowerFlow Actor that extends DataParallelPPOActor with partition function estimation."""

    def __init__(self, config, *args, **kwargs):
        super().__init__(config, *args, **kwargs)
        # PowerFlow hyperparameters: read from config, with defaults
        self.powerflow_beta_coef = getattr(config, "beta_coef", 4.0)  # β coefficient for reward scaling in powerflow loss
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                output = self.actor_module(
                    ...
                    output_hidden_states=True if return_log_z else False,  # PowerFlow: for log_z estimation
                    **multi_modal_inputs,
                    use_cache=False,
                    **extra_args,
            ...
            # ==== PowerFlow: use proj_z to estimate log Z ====
            if return_log_z:
    ...
    def update_policy(self, data: DataProto):
        ...
        mini_batches = _split_dataproto_by_size(data, self.config.ppo_mini_batch_size)
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                if self.config.use_dynamic_bsz:
                    ...
                    micro_batches = _split_dataproto_by_size(mini_batch, self.config.ppo_micro_batch_size_per_gpu)
                ...
                for micro_batch in micro_batches:
                    ...
                    boxed_reward = model_inputs["boxed_reward"]
                    ...
                    # PowerFlow: compute log probs and log Z
                    ...
                    # Compute PowerFlow trajectory balance loss
                    ...
                    policy_loss, powerflow_metrics = self.compute_powerflow(
                        ...
                        rollout_log_probs=rollout_log_probs,
                        use_boxed_reward=self.config.use_boxed_reward,
                    )
    ...
    def compute_powerflow(
        ...
        use_boxed_reward=True,
    ):
        log_ratio = log_prob - old_log_prob  # (B, T)
        ratio = torch.exp(log_ratio)  # (B, T)

        condition_1 = (ratio > 1.0 + 0.28)  # (B, T)
        condition_2 = (ratio < 1.0 - 0.2)  # (B, T)
        ...
        boxed_reward = verl_F.masked_sum(boxed_reward, combined_mask, axis=1)
        assert boxed_reward.shape == avg_ref_log_prob.shape, "boxed_reward and avg_ref_log_prob must have the same shape"
        # PowerFlow residual: logZ + logpf - β*R - logpref
        if use_boxed_reward:
            delta = log_z + avg_log_prob - self.powerflow_beta_coef * (avg_ref_log_prob + (boxed_reward - 1)/2)
        else:
        ...
        loss_term_dict = {
            ...
            "actor/boxed_reward/mean": boxed_reward.mean().detach().item(),
            "actor/boxed_reward/min": boxed_reward.min().detach().item(),
            "actor/boxed_reward/max": boxed_reward.max().detach().item(),
            "actor/total_ref_log_prob": verl_F.masked_sum(ref_log_prob, response_mask, axis=-1).detach().mean().item(),
            ...
            "actor/beta_coef": self.powerflow_beta_coef,
