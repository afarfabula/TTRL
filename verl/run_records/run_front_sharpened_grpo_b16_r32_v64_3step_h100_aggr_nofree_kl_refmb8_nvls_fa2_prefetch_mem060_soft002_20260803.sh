#!/usr/bin/env bash
set -euo pipefail

ROOT="/mlx_devbox/users/quyanyi/playground/TTRL/verl"
RUN_ID="ttrl_sharpened_grpo_b16_r32_v64_3step_h100_aggr_nofree_kl_refmb8_nvls_fa2_prefetch_mem060_soft002_20260803"
LOG="/mlx_devbox/users/quyanyi/playground/TTRL/important_experiment_logs/${RUN_ID}.log"

mkdir -p "$(dirname "$LOG")"
rm -f "$LOG"

cd "$ROOT"
RUN_ID="$RUN_ID" \
EXPERIMENT="$RUN_ID" \
LOG_NAME="0803-${RUN_ID}-MATH-TTT-Qwen2.5-Math-7B-grpo-h100-aggr" \
TTRL_RUNTIME_DIR="/tmp/ttrlklr8nvlsfa2prefetch" \
OUTPUT_DIR="/tmp/ttrl_h100/checkpoints/${RUN_ID}" \
GPU_MEMORY_UTILIZATION=0.60 \
ROLLOUT_FREE_CACHE_ENGINE=False \
ROLLOUT_MAX_NUM_BATCHED_TOKENS=8192 \
ACTOR_USE_KL_LOSS=True \
ATTN_IMPLEMENTATION=flash_attention_2 \
ACTOR_FSDP_FORWARD_PREFETCH=True \
REF_FSDP_FORWARD_PREFETCH=True \
CRITIC_FSDP_FORWARD_PREFETCH=True \
REF_LOG_PROB_MICRO_BATCH_SIZE=8 \
ROLLOUT_LOG_PROB_MICRO_BATCH_SIZE=8 \
NCCL_NVLS_ENABLE=1 \
NCCL_SOCKET_FAMILY=AF_INET6 \
TOTAL_TRAINING_STEPS=3 \
FINAL_VAL_ENABLE=False \
bash run_records/ttrl_sharpened_grpo_b16_r32_v64_20step_h100_aggr_nofree_nokl_soft002_20260803.sh 2>&1 | tee "$LOG"
