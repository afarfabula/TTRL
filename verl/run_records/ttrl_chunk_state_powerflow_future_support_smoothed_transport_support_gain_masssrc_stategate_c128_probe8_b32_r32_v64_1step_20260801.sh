...
export RUN_ID="${RUN_ID:-ttrl_chunk_state_powerflow_future_support_smoothed_transport_support_gain_masssrc_stategate_c128_probe8_b32_r32_v64_1step_20260801}"
export EXPERIMENT="${EXPERIMENT:-${RUN_ID}}"
export LOG_NAME="${LOG_NAME:-0801-${RUN_ID}-MATH-TTT-Qwen2.5-Math-7B-smoothed-transport-support-gain-masssrc-stategate-smoke}"
export TTRL_RUNTIME_DIR="${TTRL_RUNTIME_DIR:-/tmp/cfssg1}"
...
exec bash "$LAUNCHER" \
  ...
  ttrl.chunk_state_future_support_score_type=smoothed_transport_support_gain \
  ...
  ttrl.chunk_state_probe_samples=8 \
  ttrl.chunk_state_probe_max_tokens=3072 \
  ttrl.chunk_state_staged_probe_enable=False \
  ttrl.chunk_state_staged_probe_topk=0 \
  ttrl.chunk_state_staged_probe_extra_samples=0 \
  ttrl.chunk_state_staged_probe_extra_max_tokens=3072 \
