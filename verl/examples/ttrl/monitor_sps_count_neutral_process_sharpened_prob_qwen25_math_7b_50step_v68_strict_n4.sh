...
LOG_ROOT=/tmp/r68/ray/session_latest/logs
...
latest_runner_status=$(
  grep -h -a -e 'WORKER_QWEN25_MATH_7B_V68_COUNT_NEUTRAL_PROCESS_SHARPENED_STRICT_N4_' "$LOG" "$PROC_HEALTH" 2>/dev/null | tail -20 || true
)

echo "MONITOR_V68_READ_ONLY_DATE $(date '+%F %T %Z')"
...
if [[ -n "$latest_train" ]]; then
  printf 'STEP=%s MODE=%s COUNT_NEUTRAL=%s TARGET_EK=%s TARGET_CONF=%s MAJ_MASS=%s BASE_AGREE=%s PICK_ACC=%s CORRECT_MASS=%s CLIP=%s STEP_S=%s TOKENS=%s THROUGHPUT=%s\n' \
