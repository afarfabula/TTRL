...
LOG=/opt/tiger/TTRL/verl/sps_bucket_capacity_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_310step_v28.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_bucket_capacity_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_310step_v28_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_bucket_capacity_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_310step_v28_metrics.txt
RAY_DIR=/tmp/r28_310
SRC_MODEL=/opt/tiger/qwen3_8b
LOCAL_MODEL=/tmp/qwen3_8b_local_v21_answer_sharpen

echo "WORKER_QWEN3_8B_V28_310_START $(date '+%F %T')" | tee "$LOG"
...
export TORCHINDUCTOR_CACHE_DIR=/tmp/ti_qwen3_8b_v28_bucket_capacity_310
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen3_8b-sps-bucket-capacity-beta20-floor015-clip05-power15-refbase-localfp32-v28-310step-8gpu \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}

task_log=$(
  grep -R -l -a "Final validation metrics\|training/global_step:310\|val-core/MATH-TTT/acc/mean@4" \
    "$RAY_DIR/ray/session_latest/logs" 2>/dev/null | sort | head -1 || true
)
if [ -z "$task_log" ]; then
  task_log=$(
    grep -R -l -a "Training from scratch\|Total training steps: 310" \
      "$RAY_DIR/ray/session_latest/logs" 2>/dev/null | sort | head -1 || true
  )
fi
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  cp "$task_log" "$RAY_LOG_SNAPSHOT" || true
  {
    ...
    grep -a -E "training/global_step|train/sps/selected_|train/sps/selection_fallback|train/sps/selection_capacity|train/sps/answer_|train/sps/majority_sharp|train/sps/train_weight|train/sps/weighted_label_confidence|train/ground_truth_reward|train/pass@32|train/majority_ratio|response_length/clip_ratio|val-core/MATH-TTT/acc|val-aux/MATH-TTT/acc|val-aux/MATH-TTT/format_score|Final validation metrics|global_step:310" "$task_log" || true
  } > "$METRICS_SNAPSHOT"
  echo "WORKER_QWEN3_8B_V28_310_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN3_8B_V28_310_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
  tail -80 "$METRICS_SNAPSHOT" | tee -a "$LOG"
else
  echo "WORKER_QWEN3_8B_V28_310_RAY_LOG_MISSING" | tee -a "$LOG"
fi
echo "WORKER_QWEN3_8B_V28_310_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
exit "$status"
