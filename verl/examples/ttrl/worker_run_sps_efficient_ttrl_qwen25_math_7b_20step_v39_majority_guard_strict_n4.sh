...
LOG=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_20step_v39_majority_guard_strict_n4.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_20step_v39_majority_guard_strict_n4_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_20step_v39_majority_guard_strict_n4_metrics.txt
THROUGHPUT_SUMMARY=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_20step_v39_majority_guard_strict_n4_throughput_summary.txt
PROC_HEALTH=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_20step_v39_majority_guard_strict_n4_proc_health.txt
RAY_DIR=/tmp/r39n4
SRC_MODEL=/opt/tiger/qwen2.5_math_7b
LOCAL_MODEL=/tmp/qwen2_5_math_7b_local_v39_majority_guard_strict_n4_20step

final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V39_MAJORITY_GUARD_STRICT_N4_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V39_MAJORITY_GUARD_STRICT_N4_START $(date '+%F %T')" | tee "$LOG"
...
export TORCHINDUCTOR_CACHE_DIR=/tmp/ti_qwen25_math_7b_v39_majority_guard_strict_n4
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen25_math_7b-efficient-ttrl-v39-majority-guard-strict-n4 \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
set -e
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN25_MATH_7B_V39_MAJORITY_GUARD_STRICT_N4_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V39_MAJORITY_GUARD_STRICT_N4_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V39_MAJORITY_GUARD_STRICT_N4_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V39_MAJORITY_GUARD_STRICT_N4_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V39_MAJORITY_GUARD_STRICT_N4_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
