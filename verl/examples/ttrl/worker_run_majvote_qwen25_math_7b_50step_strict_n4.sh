...
EXP=majvote_qwen25_math_7b_50step_strict_n4
...
LOCAL_MODEL=/tmp/q25maj

final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_${EXP}_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_${EXP}_START $(date '+%F %T')" | tee "$LOG"
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen25_math_7b-majvote-baseline-50step-strict-n4 \
...
PY
  ...
  echo "WORKER_${EXP}_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_${EXP}_STATUS $status $(date '+%F %T')"
...
echo "WORKER_${EXP}_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
