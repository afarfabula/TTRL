...
EXP=sps_efficient_ttrl_qwen25_math_7b_50step_v70_process_consistency_sps32_selection
...
RAY_DIR=/tmp/r70sps32
SRC_MODEL=/opt/tiger/qwen2.5_math_7b
LOCAL_MODEL=/tmp/qwen2_5_math_7b_local_v70_process_consistency_sps32_50step

final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V70_PROCESS_CONSISTENCY_SPS32_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V70_PROCESS_CONSISTENCY_SPS32_START $(date '+%F %T')" | tee "$LOG"
...
ttrl_setup_cuda_env "qwen25_v70_process_consistency_sps32_50step" "$LOG"
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen25_math_7b-efficient-ttrl-v70-process-consistency-sps32-selection \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
set -e
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN25_MATH_7B_V70_PROCESS_CONSISTENCY_SPS32_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V70_PROCESS_CONSISTENCY_SPS32_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V70_PROCESS_CONSISTENCY_SPS32_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V70_PROCESS_CONSISTENCY_SPS32_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V70_PROCESS_CONSISTENCY_SPS32_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
