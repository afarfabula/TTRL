...
EXP=sps_efficient_ttrl_qwen25_math_7b_50step_v58_process_answer_reward_strict_n4
...
# Keep Ray's AF_UNIX socket path short; the experiment cache root is too long
# once Ray appends session/sockets/plasma_store.
RAY_DIR=/tmp/r58
# Python multiprocessing also creates AF_UNIX sockets under TMPDIR when
# sharing torch storages, so TMPDIR must be short as well.
SHORT_TMPDIR=/tmp/t58
...
final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V58_PROCESS_ANSWER_REWARD_STRICT_N4_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V58_PROCESS_ANSWER_REWARD_STRICT_N4_START $(date '+%F %T')" | tee "$LOG"
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  ttrl.sps_support_conflict_capacity=False \
  ttrl.sps_independent_support_reward=False \
  ttrl.sps_independent_support_value=0.18 \
  ttrl.sps_independent_support_min_base=0.12 \
  ttrl.sps_independent_support_min_process=0.65 \
  ttrl.sps_independent_support_min_cluster_mass=0.0625 \
  ttrl.sps_independent_support_min_support=0.20 \
  ttrl.sps_independent_support_entropy_gate=1.15 \
  ttrl.sps_independent_support_allow_majority=True \
  ttrl.sps_independent_support_majority_margin_max=0.12 \
  ttrl.sps_independent_support_competition_min=0.35 \
  ttrl.sps_process_answer_reward=True \
  ttrl.sps_process_answer_value=0.16 \
  ttrl.sps_process_answer_min_base=0.10 \
  ttrl.sps_process_answer_min_process=0.60 \
  ttrl.sps_process_answer_min_support=0.20 \
  ttrl.sps_process_answer_majority_scale=0.50 \
  ...
  trainer.experiment_name=math-qwen25_math_7b-efficient-ttrl-v58-process-answer-reward-strict-n4 \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
set -e
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN25_MATH_7B_V58_PROCESS_ANSWER_REWARD_STRICT_N4_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V58_PROCESS_ANSWER_REWARD_STRICT_N4_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V58_PROCESS_ANSWER_REWARD_STRICT_N4_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V58_PROCESS_ANSWER_REWARD_STRICT_N4_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V58_PROCESS_ANSWER_REWARD_STRICT_N4_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
