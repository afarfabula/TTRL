...
EXP=sps_efficient_ttrl_qwen25_math_7b_50step_v52_anti_collapse_capacity_strict_n4
...
LOCAL_MODEL=/tmp/qwen2_5_math_7b_local_v52_anti_collapse_capacity_strict_n4_50step

final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V52_ANTI_COLLAPSE_CAPACITY_STRICT_N4_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V52_ANTI_COLLAPSE_CAPACITY_STRICT_N4_START $(date '+%F %T')" | tee "$LOG"
...
ttrl_setup_cuda_env "qwen25_v52_anti_collapse_capacity_strict_n4_50step" "$LOG"
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  ttrl.sps_anti_collapse_capacity=True \
  ttrl.sps_anti_collapse_min_weight=0.45 \
  ttrl.sps_anti_collapse_sharp_min=0.95 \
  ttrl.sps_anti_collapse_effective_k_max=1.25 \
  ttrl.sps_anti_collapse_low_budget_mass_min=0.75 \
  ttrl.sps_anti_collapse_margin_gap_min=0.15 \
  ...
  trainer.experiment_name=math-qwen25_math_7b-efficient-ttrl-v52-anti-collapse-capacity-strict-n4 \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
set -e
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN25_MATH_7B_V52_ANTI_COLLAPSE_CAPACITY_STRICT_N4_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V52_ANTI_COLLAPSE_CAPACITY_STRICT_N4_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V52_ANTI_COLLAPSE_CAPACITY_STRICT_N4_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V52_ANTI_COLLAPSE_CAPACITY_STRICT_N4_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V52_ANTI_COLLAPSE_CAPACITY_STRICT_N4_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
