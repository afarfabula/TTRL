...
LOG=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_50step_v47_base_supported_repair_strict_n4.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_50step_v47_base_supported_repair_strict_n4_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_50step_v47_base_supported_repair_strict_n4_metrics.txt
THROUGHPUT_SUMMARY=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_50step_v47_base_supported_repair_strict_n4_throughput_summary.txt
PROC_HEALTH=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_50step_v47_base_supported_repair_strict_n4_proc_health.txt
RAY_DIR=/tmp/r47basesup
SRC_MODEL=/opt/tiger/qwen2.5_math_7b
LOCAL_MODEL=/tmp/qwen2_5_math_7b_local_v47_base_supported_repair_strict_n4_50step

final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V47_BASE_SUPPORTED_REPAIR_STRICT_N4_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V47_BASE_SUPPORTED_REPAIR_STRICT_N4_START $(date '+%F %T')" | tee "$LOG"
...
ttrl_setup_cuda_env "qwen25_v47_base_supported_repair_strict_n4_50step" "$LOG"
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  ttrl.sps_rollout_selection=base_supported_repair \
  ttrl.sps_low_budget_repair_max_replacements=4 \
  ttrl.sps_base_supported_repair_min_gain=0.0 \
  ...
  trainer.experiment_name=math-qwen25_math_7b-efficient-ttrl-v47-base-supported-repair-strict-n4 \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
set -e
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN25_MATH_7B_V47_BASE_SUPPORTED_REPAIR_STRICT_N4_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V47_BASE_SUPPORTED_REPAIR_STRICT_N4_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V47_BASE_SUPPORTED_REPAIR_STRICT_N4_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V47_BASE_SUPPORTED_REPAIR_STRICT_N4_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V47_BASE_SUPPORTED_REPAIR_STRICT_N4_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
