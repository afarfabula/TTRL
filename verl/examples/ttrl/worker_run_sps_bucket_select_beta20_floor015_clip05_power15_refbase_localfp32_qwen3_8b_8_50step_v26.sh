...
LOG=/opt/tiger/TTRL/verl/sps_bucket_select_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_50step_v26.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_bucket_select_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_v26_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_bucket_select_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_v26_metrics.txt
RAY_DIR=/tmp/r26
SRC_MODEL=/opt/tiger/qwen3_8b
LOCAL_MODEL=/tmp/qwen3_8b_local_v21_answer_sharpen

echo "WORKER_QWEN3_8B_V26_START $(date '+%F %T')" | tee "$LOG"
...
export TORCHINDUCTOR_CACHE_DIR=/tmp/ti_qwen3_8b_v26_bucket_select
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen3_8b-sps-bucket-select-beta20-floor015-clip05-power15-refbase-localfp32-v26-50step-8gpu \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN3_8B_V26_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN3_8B_V26_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
  tail -80 "$METRICS_SNAPSHOT" | tee -a "$LOG"
else
  echo "WORKER_QWEN3_8B_V26_RAY_LOG_MISSING" | tee -a "$LOG"
fi
echo "WORKER_QWEN3_8B_V26_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
exit "$status"
