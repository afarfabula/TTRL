#!/bin/bash
# SPS confidence-weighted majority TTRL on MATH-TTT (Math500), 8x GPU.
...
#   4. Multiply the SPS training reward by a continuous prompt weight derived
#      from majority confidence, SPS-majority agreement confidence, and an
#      internal truncation penalty. No ground-truth labels are used for training.
set -x
...
export RAY_TMPDIR=${RAY_TMPDIR:-/tmp/ray_sps_conf_weight8}
...
$PY -m verl.trainer.main_ppo \
  ...
  trainer.experiment_name=math-qwen3_4b-sps-conf-weight-50step-8gpu \
