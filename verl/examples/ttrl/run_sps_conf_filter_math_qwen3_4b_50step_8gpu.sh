#!/bin/bash
# SPS confidence-filtered majority TTRL on MATH-TTT (Math500), 8x GPU.
...
#   3. Compute the answer-level SPS label/confidence from base/proposal logprobs.
#   4. Train only prompts with strong majority confidence or SPS-majority
#      agreement confidence. No ground-truth labels are used for training.
set -x
...
export RAY_TMPDIR=${RAY_TMPDIR:-/tmp/ray_sps_conf_filter8}
...
$PY -m verl.trainer.main_ppo \
  ...
  ttrl.sps_filter_confidence_threshold=$SPS_FILTER_CONF \
  ttrl.sps_filter_majority_ratio_threshold=$SPS_FILTER_MAJ \
  ttrl.n_votes_per_prompt=$N_VOTES \
  ttrl.n_samples_per_prompt=$N_SAMPLES \
  trainer.logger=['console'] \
  trainer.project_name=TTRL-SPS \
  trainer.experiment_name=math-qwen3_4b-sps-conf-filter-50step-8gpu \
