...
LOG=/opt/tiger/TTRL/verl/sps_rule_conf_weight_floor015_clip05_power15_refbase_localfp32_qwen25_math_7b_8_184step_v14.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_rule_conf_weight_floor015_clip05_power15_refbase_localfp32_qwen25_math_7b_8_184step_v14_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_rule_conf_weight_floor015_clip05_power15_refbase_localfp32_qwen25_math_7b_8_184step_v14_metrics.txt
RAY_DIR=/tmp/r184_v14_qwen25_math_7b
SRC_MODEL=/opt/tiger/qwen2.5_math_7b
LOCAL_MODEL=/tmp/qwen2.5_math_7b_local_184_v14

echo "WORKER_QWEN25_MATH_7B_V14_184_START $(date '+%F %T')" | tee "$LOG"
...
export TORCHINDUCTOR_CACHE_DIR=/tmp/ti_qwen25_math_7b_v14_184
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen25_math_7b-sps-rule-conf-weight-floor015-clip05-power15-refbase-localfp32-v14-184step-8gpu \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN25_MATH_7B_V14_184_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V14_184_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
  tail -80 "$METRICS_SNAPSHOT" | tee -a "$LOG"
else
  echo "WORKER_QWEN25_MATH_7B_V14_184_RAY_LOG_MISSING" | tee -a "$LOG"
fi
echo "WORKER_QWEN25_MATH_7B_V14_184_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
exit "$status"
