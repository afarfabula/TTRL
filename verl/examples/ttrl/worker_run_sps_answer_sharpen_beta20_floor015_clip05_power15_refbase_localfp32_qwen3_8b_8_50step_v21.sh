...
LOG=/opt/tiger/TTRL/verl/sps_answer_sharpen_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_50step_v21.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_answer_sharpen_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_v21_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_answer_sharpen_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_v21_metrics.txt
RAY_DIR=/tmp/r21_qwen3_8b_answer_sharpen
SRC_MODEL=/opt/tiger/qwen3_8b
LOCAL_MODEL=/tmp/qwen3_8b_local_v21_answer_sharpen

echo "WORKER_QWEN3_8B_V21_START $(date '+%F %T')" | tee "$LOG"
...
export TORCHINDUCTOR_CACHE_DIR=/tmp/ti_qwen3_8b_v21_answer_sharpen
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen3_8b-sps-answer-sharpen-beta20-floor015-clip05-power15-refbase-localfp32-v21-50step-8gpu \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  cp "$task_log" "$RAY_LOG_SNAPSHOT" || true
  {
    ...
    grep -a -E "training/global_step|train/sps/answer_|train/sps/majority_sharp|train/sps/train_weight|val-core/MATH-TTT/acc|val-aux/MATH-TTT/acc|Final validation metrics|global_step:50" "$task_log" || true
  } > "$METRICS_SNAPSHOT"
  echo "WORKER_QWEN3_8B_V21_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN3_8B_V21_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
  tail -80 "$METRICS_SNAPSHOT" | tee -a "$LOG"
else
  echo "WORKER_QWEN3_8B_V21_RAY_LOG_MISSING" | tee -a "$LOG"
fi
echo "WORKER_QWEN3_8B_V21_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
exit "$status"
