...
EXP=${EXP:-sps_efficient_ttrl_qwen25_math_7b_30step_v48_process_consistency_strict_n4_save_tmp}
...
RAY_DIR=${RAY_DIR:-/tmp/r48p30}
TMP_RUNTIME_DIR=${TMP_RUNTIME_DIR:-/tmp/r48p30t}
SRC_MODEL=/opt/tiger/qwen2.5_math_7b
LOCAL_MODEL=${LOCAL_MODEL:-/tmp/qwen2_5_math_7b_local_v48_process_consistency_strict_n4_30step}
CKPT_DIR=${CKPT_DIR:-/tmp/ttrl_ckpts/qwen25_math_7b_v48_process_consistency_strict_n4_30step}

final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V48_PROCESS_CONSISTENCY_STRICT_N4_30STEP_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V48_PROCESS_CONSISTENCY_STRICT_N4_30STEP_START $(date '+%F %T')" | tee "$LOG"
...
ttrl_setup_cuda_env "qwen25_v48_process_consistency_strict_n4_30step_save_tmp" "$LOG"
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen25_math_7b-efficient-ttrl-v48-process-consistency-strict-n4-30step-save-tmp \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
set -e

task_log=$(
  grep -R -l -a "training/global_step:30\|Final validation metrics\|Total training steps: 30" \
    "$RAY_DIR/ray/session_latest/logs" 2>/dev/null | sort | head -1 || true
)
if [ -z "$task_log" ]; then
  task_log=$(
    grep -R -l -a "Training from scratch\|training/global_step" \
      "$RAY_DIR/ray/session_latest/logs" 2>/dev/null | sort | head -1 || true
  )
fi
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  cp "$task_log" "$RAY_LOG_SNAPSHOT" || true
  {
    ...
    grep -a -E "training/global_step|train/sps/|train/label_accuracy|train/reward_accuracy|train/majority_voting_reward|train/ground_truth_reward|train/pass@32|train/majority_ratio|response_length/clip_ratio|response_avg_logprob|response_clip|timing_s/|perf/total_num_tokens|perf/throughput|Final validation metrics|global_step:30" "$task_log" || true
  } > "$METRICS_SNAPSHOT"
  echo "WORKER_QWEN25_MATH_7B_V48_PROCESS_CONSISTENCY_STRICT_N4_30STEP_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V48_PROCESS_CONSISTENCY_STRICT_N4_30STEP_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V48_PROCESS_CONSISTENCY_STRICT_N4_30STEP_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V48_PROCESS_CONSISTENCY_STRICT_N4_30STEP_STATUS $status $(date '+%F %T')"
...
if [ -d "$CKPT_DIR/global_step_30/actor" ]; then
  echo "WORKER_QWEN25_MATH_7B_V48_PROCESS_CONSISTENCY_STRICT_N4_30STEP_CKPT $CKPT_DIR/global_step_30" | tee -a "$LOG"
else
  echo "WORKER_QWEN25_MATH_7B_V48_PROCESS_CONSISTENCY_STRICT_N4_30STEP_CKPT_MISSING $CKPT_DIR/global_step_30" | tee -a "$LOG"
fi

echo "WORKER_QWEN25_MATH_7B_V48_PROCESS_CONSISTENCY_STRICT_N4_30STEP_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
