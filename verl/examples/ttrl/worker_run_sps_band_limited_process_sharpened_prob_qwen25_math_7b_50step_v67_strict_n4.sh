...
EXP=sps_band_limited_process_sharpened_prob_qwen25_math_7b_50step_v67_strict_n4
...
final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V67_BAND_LIMITED_PROCESS_SHARPENED_STRICT_N4_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V67_BAND_LIMITED_PROCESS_SHARPENED_STRICT_N4_START $(date '+%F %T')" | tee "$LOG"
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen25_math_7b-band-limited-process-sharpened-prob-v67-strict-n4 \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
set -e
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN25_MATH_7B_V67_BAND_LIMITED_PROCESS_SHARPENED_STRICT_N4_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V67_BAND_LIMITED_PROCESS_SHARPENED_STRICT_N4_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V67_BAND_LIMITED_PROCESS_SHARPENED_STRICT_N4_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V67_BAND_LIMITED_PROCESS_SHARPENED_STRICT_N4_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V67_BAND_LIMITED_PROCESS_SHARPENED_STRICT_N4_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
