#!/bin/bash
# SPS-gated self-consistency TTRL on MATH-TTT (Math500), 4x GPU (0/1/2/3).
# Final-only validation: val_before_train=false, test_freq=50.
#
# Internal-feedback algorithm:
#   1. Sample 64 high-temperature rollouts per prompt.
#   2. Compute both ordinary majority-vote label and answer-level SPS label.
#   3. Use the majority label by default.
#   4. Let SPS override only when SPS confidence is high and majority confidence
#      is low. No ground-truth labels are used for training.
set -x
...
export RAY_TMPDIR=${RAY_TMPDIR:-/tmp/ray_sps_weighted_gate}
...
$PY -m verl.trainer.main_ppo \
  ...
  ttrl.sps_gate_confidence_threshold=$SPS_GATE_CONF \
  ttrl.sps_gate_majority_ratio_threshold=$SPS_GATE_MAJ \
  ttrl.n_votes_per_prompt=$N_VOTES \
  ttrl.n_samples_per_prompt=$N_SAMPLES \
  trainer.logger=['console'] \
  trainer.project_name=TTRL-SPS \
  trainer.experiment_name=math-qwen3_4b-sps-weighted-gate-50step \
