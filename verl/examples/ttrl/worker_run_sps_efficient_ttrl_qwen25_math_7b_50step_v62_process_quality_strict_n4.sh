...
EXP=sps_efficient_ttrl_qwen25_math_7b_50step_v62_process_quality_strict_n4
...
final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V62_PROCESS_QUALITY_STRICT_N4_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V62_PROCESS_QUALITY_STRICT_N4_START $(date '+%F %T')" | tee "$LOG"
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  ttrl.sps_low_budget_negative_reward=False \
  ttrl.sps_low_budget_soft_reward=False \
  ...
  ttrl.sps_process_answer_reward=False \
  ttrl.sps_contrastive_alt_reward=False \
  ttrl.sps_entropy_band_capacity=False \
  ttrl.sps_entropy_band_min_weight=0.45 \
  ttrl.sps_entropy_band_low_k=1.25 \
  ttrl.sps_entropy_band_high_k=4.0 \
  ttrl.sps_entropy_band_sharp_min=0.92 \
  ttrl.sps_entropy_band_low_budget_mass_min=0.70 \
  ttrl.sps_entropy_band_support_min=0.70 \
  ttrl.sps_low_budget_rebalance_reward=False \
  ttrl.sps_low_budget_rebalance_alt_value=0.10 \
  ttrl.sps_low_budget_rebalance_majority_penalty=0.04 \
  ttrl.sps_low_budget_rebalance_sharp_min=0.92 \
  ttrl.sps_low_budget_rebalance_effective_k_max=1.35 \
  ttrl.sps_low_budget_rebalance_mass_min=0.70 \
  ttrl.sps_low_budget_rebalance_support_min=0.70 \
  ...
  ttrl.sps_process_quality_reward=True \
  ...
  trainer.experiment_name=math-qwen25_math_7b-efficient-ttrl-v62-process-quality-strict-n4 \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
set -e
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  cp "$task_log" "$RAY_LOG_SNAPSHOT" || true
  {
    ...
    grep -a -E "training/global_step|train/sps/|train/sps_process_quality|train/sps_low_budget_rebalance|train/sps_entropy_band|train/label_accuracy|train/reward_accuracy|train/majority_voting_reward|train/ground_truth_reward|train/pass@32|train/majority_ratio|response_length/clip_ratio|response_avg_logprob|response_clip|timing_s/|perf/total_num_tokens|perf/throughput|val-core/MATH-TTT/acc|val-aux/MATH-TTT/(acc|format_score|response_len|response_clip|response_avg_logprob)|Final validation metrics|global_step:50" "$task_log" || true
  } > "$METRICS_SNAPSHOT"
  echo "WORKER_QWEN25_MATH_7B_V62_PROCESS_QUALITY_STRICT_N4_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V62_PROCESS_QUALITY_STRICT_N4_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V62_PROCESS_QUALITY_STRICT_N4_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V62_PROCESS_QUALITY_STRICT_N4_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V62_PROCESS_QUALITY_STRICT_N4_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
