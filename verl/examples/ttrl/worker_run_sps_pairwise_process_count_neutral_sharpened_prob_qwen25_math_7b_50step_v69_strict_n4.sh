...
final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V69_PAIRWISE_PROCESS_COUNT_NEUTRAL_SHARPENED_STRICT_N4_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V69_PAIRWISE_PROCESS_COUNT_NEUTRAL_SHARPENED_STRICT_N4_START $(date '+%F %T')" | tee "$LOG"
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen25_math_7b-pairwise-process-count-neutral-sharpened-prob-v69-strict-n4 \
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN25_MATH_7B_V69_PAIRWISE_PROCESS_COUNT_NEUTRAL_SHARPENED_STRICT_N4_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V69_PAIRWISE_PROCESS_COUNT_NEUTRAL_SHARPENED_STRICT_N4_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V69_PAIRWISE_PROCESS_COUNT_NEUTRAL_SHARPENED_STRICT_N4_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V69_PAIRWISE_PROCESS_COUNT_NEUTRAL_SHARPENED_STRICT_N4_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V69_PAIRWISE_PROCESS_COUNT_NEUTRAL_SHARPENED_STRICT_N4_ABLATION_SUMMARY_BEGIN $(date '+%F %T')" | tee -a "$LOG"
set +e
...
echo "WORKER_QWEN25_MATH_7B_V69_PAIRWISE_PROCESS_COUNT_NEUTRAL_SHARPENED_STRICT_N4_ABLATION_SUMMARY_STATUS ${summary_status}" | tee -a "$LOG"
echo "WORKER_QWEN25_MATH_7B_V69_PAIRWISE_PROCESS_COUNT_NEUTRAL_SHARPENED_STRICT_N4_COMPLETION_AUDIT_BEGIN $(date '+%F %T')" | tee -a "$LOG"
/opt/tiger/modelchef/.venv/bin/python3 \
  /opt/tiger/TTRL/verl/examples/ttrl/check_pure_sharpening_goal_completion.py \
  2>&1 | tee -a "$LOG"
completion_audit_status=${PIPESTATUS[0]}
echo "WORKER_QWEN25_MATH_7B_V69_PAIRWISE_PROCESS_COUNT_NEUTRAL_SHARPENED_STRICT_N4_COMPLETION_AUDIT_STATUS ${completion_audit_status}" | tee -a "$LOG"
set -e

echo "WORKER_QWEN25_MATH_7B_V69_PAIRWISE_PROCESS_COUNT_NEUTRAL_SHARPENED_STRICT_N4_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
