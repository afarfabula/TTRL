...
EXP=sps_efficient_ttrl_qwen25_math_7b_50step_v59_contrastive_alt_strict_n4
...
final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V59_CONTRASTIVE_ALT_STRICT_N4_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V59_CONTRASTIVE_ALT_STRICT_N4_START $(date '+%F %T')" | tee "$LOG"
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  ttrl.sps_contrastive_alt_reward=True \
  ttrl.sps_contrastive_alt_value=0.14 \
  ttrl.sps_contrastive_alt_majority_penalty=0.06 \
  ttrl.sps_contrastive_alt_min_base=0.08 \
  ttrl.sps_contrastive_alt_min_process=0.60 \
  ttrl.sps_contrastive_alt_min_support=0.18 \
  ttrl.sps_contrastive_alt_sharp_min=0.92 \
  ttrl.sps_contrastive_alt_effective_k_max=1.35 \
  ttrl.sps_contrastive_alt_low_budget_mass_min=0.70 \
  ttrl.sps_contrastive_alt_majority_support_min=0.70 \
  ...
  trainer.experiment_name=math-qwen25_math_7b-efficient-ttrl-v59-contrastive-alt-strict-n4 \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
set -e
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  cp "$task_log" "$RAY_LOG_SNAPSHOT" || true
  {
    ...
    grep -a -E "training/global_step|train/sps/|train/sps_contrastive_alt|train/label_accuracy|train/reward_accuracy|train/majority_voting_reward|train/ground_truth_reward|train/pass@32|train/majority_ratio|response_length/clip_ratio|response_avg_logprob|response_clip|timing_s/|perf/total_num_tokens|perf/throughput|val-core/MATH-TTT/acc|val-aux/MATH-TTT/(acc|format_score|response_len|response_clip|response_avg_logprob)|Final validation metrics|global_step:50" "$task_log" || true
  } > "$METRICS_SNAPSHOT"
  echo "WORKER_QWEN25_MATH_7B_V59_CONTRASTIVE_ALT_STRICT_N4_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V59_CONTRASTIVE_ALT_STRICT_N4_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V59_CONTRASTIVE_ALT_STRICT_N4_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V59_CONTRASTIVE_ALT_STRICT_N4_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V59_CONTRASTIVE_ALT_STRICT_N4_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
