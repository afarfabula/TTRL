...
LOG=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen3_4b_20step_v31.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen3_4b_20step_v31_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen3_4b_20step_v31_metrics.txt
THROUGHPUT_SUMMARY=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen3_4b_20step_v31_throughput_summary.txt
PROC_HEALTH=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen3_4b_20step_v31_proc_health.txt
RAY_DIR=/tmp/r31_qwen3_4b_efficient_ttrl_20step
SRC_MODEL=/mnt/hdfs/models/qwen3_4b
LOCAL_MODEL=/tmp/qwen3_4b_local_v31_efficient_ttrl_20step

echo "WORKER_QWEN3_4B_V31_EFFICIENT20_START $(date '+%F %T')" | tee "$LOG"
...
export MASTER_PORT=29691
export GLOO_SOCKET_IFNAME=eth0
export NCCL_SOCKET_IFNAME=eth0
export TP_SOCKET_IFNAME=eth0
export NCCL_SOCKET_FAMILY=AF_INET6
export TORCHINDUCTOR_CACHE_DIR=/tmp/ti_qwen3_4b_v31_efficient20
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen3_4b-efficient-ttrl-v31-20step-sps-majority-sharpen \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  cp "$task_log" "$RAY_LOG_SNAPSHOT" || true
  {
    ...
    grep -a -E "training/global_step|train/sps/|train/label_accuracy|train/reward_accuracy|train/majority_voting_reward|train/ground_truth_reward|train/pass@32|train/majority_ratio|response_length/clip_ratio|timing_s/|perf/total_num_tokens|perf/throughput|val-core/MATH-TTT/acc|val-aux/MATH-TTT/acc|Final validation metrics|global_step:20" "$task_log" || true
  } > "$METRICS_SNAPSHOT"
  echo "WORKER_QWEN3_4B_V31_EFFICIENT20_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN3_4B_V31_EFFICIENT20_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
  tail -120 "$METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN3_4B_V31_EFFICIENT20_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN3_4B_V31_EFFICIENT20_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN3_4B_V31_EFFICIENT20_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
exit "$status"
