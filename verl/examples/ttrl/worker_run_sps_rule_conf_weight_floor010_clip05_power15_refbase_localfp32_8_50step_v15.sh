...
LOG=/opt/tiger/TTRL/verl/sps_rule_conf_weight_floor010_clip05_power15_refbase_localfp32_8_50step_v15.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_rule_conf_weight_floor010_clip05_power15_refbase_localfp32_8_v15_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_rule_conf_weight_floor010_clip05_power15_refbase_localfp32_8_v15_metrics.txt
RAY_DIR=/tmp/r15
SRC_MODEL=/mnt/hdfs/models/qwen3_4b
LOCAL_MODEL=/tmp/qwen3_4b_local_v15

echo "WORKER_V15_START $(date '+%F %T')" | tee "$LOG"
...
export TORCHINDUCTOR_CACHE_DIR=/tmp/ti_v15_floor010_power15_refbase_localfp32
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen3_4b-sps-rule-conf-weight-floor010-clip05-power15-refbase-localfp32-v15-50step-8gpu \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_V15_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_V15_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
  tail -80 "$METRICS_SNAPSHOT" | tee -a "$LOG"
else
  echo "WORKER_V15_RAY_LOG_MISSING" | tee -a "$LOG"
fi
echo "WORKER_V15_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
exit "$status"
