...
LOG=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen3_4b_20step_v32.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen3_4b_20step_v32_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen3_4b_20step_v32_metrics.txt
THROUGHPUT_SUMMARY=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen3_4b_20step_v32_throughput_summary.txt
PROC_HEALTH=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen3_4b_20step_v32_proc_health.txt
RAY_DIR=/tmp/r32
SRC_MODEL=/mnt/hdfs/models/qwen3_4b
LOCAL_MODEL=/tmp/qwen3_4b_local_v32_efficient_ttrl_20step

echo "WORKER_QWEN3_4B_V32_EFFICIENT20_START $(date '+%F %T')" | tee "$LOG"
...
export MASTER_PORT=29692
export GLOO_SOCKET_IFNAME=eth0
export NCCL_SOCKET_IFNAME=eth0
export TP_SOCKET_IFNAME=eth0
export NCCL_SOCKET_FAMILY=AF_INET6
export TORCHINDUCTOR_CACHE_DIR=/tmp/ti_qwen3_4b_v32_efficient20
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen3_4b-efficient-ttrl-v32-20step-v14-capacity-sharpen \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN3_4B_V32_EFFICIENT20_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN3_4B_V32_EFFICIENT20_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN3_4B_V32_EFFICIENT20_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN3_4B_V32_EFFICIENT20_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN3_4B_V32_EFFICIENT20_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
exit "$status"
