...
LOG=/opt/tiger/TTRL/verl/sps_rule_conf_weight_floor015_clip05_refbase_localbf16_8_50step_v13e.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_rule_conf_weight_floor015_clip05_refbase_localbf16_8_v13e_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_rule_conf_weight_floor015_clip05_refbase_localbf16_8_v13e_metrics.txt
RAY_DIR=/tmp/ray_v13e_refbase_localbf16
SRC_MODEL=/mnt/hdfs/models/qwen3_4b
LOCAL_MODEL=/tmp/qwen3_4b_local_v13e

echo "WORKER_V13E_START $(date '+%F %T')" | tee "$LOG"
...
export TORCHINDUCTOR_CACHE_DIR=/tmp/ti_v13e_refbase_localbf16
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen3_4b-sps-rule-conf-weight-floor015-clip05-refbase-localbf16-v13e-50step-8gpu \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_V13E_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_V13E_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
  tail -80 "$METRICS_SNAPSHOT" | tee -a "$LOG"
else
  echo "WORKER_V13E_RAY_LOG_MISSING" | tee -a "$LOG"
fi
echo "WORKER_V13E_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
exit "$status"
