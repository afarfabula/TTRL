...
LOG=/opt/tiger/TTRL/verl/sps_rule_conf_weight_clip05_8_50step.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_rule_conf_weight_clip05_8_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_rule_conf_weight_clip05_8_metrics.txt
RAY_DIR=/tmp/ray_sps_rule_conf_weight_clip05_8

echo "WORKER_V7_START $(date '+%F %T')" | tee "$LOG"
...
export TORCHINDUCTOR_CACHE_DIR=/tmp/torchinductor_sps_rule_conf_weight_clip05_8
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen3_4b-sps-rule-conf-weight-clip05-50step-8gpu \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_V7_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_V7_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
  tail -80 "$METRICS_SNAPSHOT" | tee -a "$LOG"
else
  echo "WORKER_V7_RAY_LOG_MISSING" | tee -a "$LOG"
fi
echo "WORKER_V7_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
exit "$status"
