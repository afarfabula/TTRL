#!/bin/bash
# SPS confidence-weighted majority TTRL with fixed SPS reward union and
# mixed self-consistency reward. Final-only validation.
set -x
...
export TORCHINDUCTOR_CACHE_DIR=${TORCHINDUCTOR_CACHE_DIR:-/tmp/torchinductor_sps_mix_fixed8}

export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}
export RAY_TMPDIR=${RAY_TMPDIR:-/tmp/ray_sps_conf_mix_fixed8}
...
$PY -m verl.trainer.main_ppo \
  ...
  ttrl.sps_majority_reward_coef=$SPS_MAJORITY_REWARD_COEF \
  ttrl.n_votes_per_prompt=$N_VOTES \
  ttrl.n_samples_per_prompt=$N_SAMPLES \
  trainer.logger=['console'] \
  trainer.project_name=TTRL-SPS \
  trainer.experiment_name=math-qwen3_4b-sps-conf-weight-majority-mix-fixed-50step-8gpu \
