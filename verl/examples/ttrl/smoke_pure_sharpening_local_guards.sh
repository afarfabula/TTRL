...
echo "SMOKE_PURE_SHARPENING_LOCAL_GUARDS_BEGIN $(date '+%F %T')"

echo "SMOKE py_compile"
"$PY" -m py_compile \
  "$ROOT/examples/ttrl/audit_pure_sharpening_docs.py" \
  ...
  "$ROOT/examples/ttrl/smoke_v69_pairwise_target_cpu.py" \
...
bash -n \
  "$ROOT/examples/ttrl/check_worker_readiness_for_v69.sh" \
  "$ROOT/examples/ttrl/check_worker_readiness_for_v81.sh" \
  "$ROOT/examples/ttrl/guarded_run_v69_pairwise_pure_sharpening.sh" \
  "$ROOT/examples/ttrl/smoke_worker_readiness_guard.sh" \
  "$ROOT/examples/ttrl/smoke_v81_worker_readiness_guard.sh" \
  "$ROOT/examples/ttrl/smoke_v81_local_guards.sh" \
  "$ROOT/examples/ttrl/worker_run_sps_count_neutral_process_sharpened_prob_qwen25_math_7b_50step_v68_strict_n4.sh" \
  "$ROOT/examples/ttrl/worker_run_sps_pairwise_process_count_neutral_sharpened_prob_qwen25_math_7b_50step_v69_strict_n4.sh" \
  "$ROOT/examples/ttrl/worker_run_sps_basin_contrast_process_sharpened_prob_qwen25_math_7b_50step_v81_strict_n4.sh"
...
echo "SMOKE doc_audit"
"$PY" "$ROOT/examples/ttrl/audit_pure_sharpening_docs.py"

echo "SMOKE v69_pairwise_target_cpu"
"$PY" "$ROOT/examples/ttrl/smoke_v69_pairwise_target_cpu.py"

echo "SMOKE v81_basin_contrast_cpu"
"$PY" "$ROOT/examples/ttrl/smoke_v81_basin_contrast_cpu.py"

echo "SMOKE ablation_summary"
"$PY" "$ROOT/examples/ttrl/summarize_pure_sharpening_ablation.py" \
  | tee "$ROOT/pure_sharpening_ablation_summary.tsv"
...
if [ "$completion_status" -eq 0 ]; then
  echo "SMOKE completion_gate_passed"
else
  echo "SMOKE completion_gate_not_complete"
fi

echo "SMOKE worker_readiness_read_only"
set +e
...
echo "SMOKE worker_readiness_guard_fixtures"
bash "$ROOT/examples/ttrl/smoke_worker_readiness_guard.sh"

echo "SMOKE v81_worker_readiness_guard_fixtures"
bash "$ROOT/examples/ttrl/smoke_v81_worker_readiness_guard.sh"

echo "SMOKE_PURE_SHARPENING_LOCAL_GUARDS_DONE $(date '+%F %T')"
