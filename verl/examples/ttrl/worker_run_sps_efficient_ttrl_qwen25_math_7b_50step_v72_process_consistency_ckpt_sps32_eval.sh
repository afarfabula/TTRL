...
EXP=${EXP:-sps_efficient_ttrl_qwen25_math_7b_50step_v72_process_consistency_ckpt_sps32_eval}
...
RAY_DIR=${RAY_DIR:-/tmp/r72sps32eval}
SRC_MODEL=/opt/tiger/qwen2.5_math_7b
LOCAL_MODEL=${LOCAL_MODEL:-/tmp/qwen2_5_math_7b_local_v72_process_consistency_sps32_eval}
...
CKPT_PATH=${CKPT_PATH:-$CKPT_DIR/global_step_50}

final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V72_PROCESS_CONSISTENCY_CKPT_SPS32_EVAL_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V72_PROCESS_CONSISTENCY_CKPT_SPS32_EVAL_START $(date '+%F %T')" | tee "$LOG"
...
ttrl_setup_cuda_env "qwen25_v72_process_consistency_ckpt_sps32_eval" "$LOG"
...
if [ ! -d "$CKPT_PATH/actor" ]; then
  echo "WORKER_QWEN25_MATH_7B_V72_PROCESS_CONSISTENCY_CKPT_SPS32_EVAL_CKPT_MISSING $CKPT_PATH" | tee -a "$LOG"
  exit 2
fi
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen25_math_7b-efficient-ttrl-v72-process-consistency-ckpt-sps32-eval \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
set -e
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN25_MATH_7B_V72_PROCESS_CONSISTENCY_CKPT_SPS32_EVAL_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V72_PROCESS_CONSISTENCY_CKPT_SPS32_EVAL_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V72_PROCESS_CONSISTENCY_CKPT_SPS32_EVAL_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V72_PROCESS_CONSISTENCY_CKPT_SPS32_EVAL_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V72_PROCESS_CONSISTENCY_CKPT_SPS32_EVAL_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
