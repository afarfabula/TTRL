...
LOG=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_20step_v33.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_20step_v33_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_20step_v33_metrics.txt
THROUGHPUT_SUMMARY=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_20step_v33_throughput_summary.txt
PROC_HEALTH=/opt/tiger/TTRL/verl/sps_efficient_ttrl_qwen25_math_7b_20step_v33_proc_health.txt
RAY_DIR=/tmp/r25v33
SRC_MODEL=/opt/tiger/qwen2.5_math_7b
LOCAL_MODEL=/tmp/qwen2_5_math_7b_local_v33_efficient_ttrl_20step

echo "WORKER_QWEN25_MATH_7B_V33_EFFICIENT20_START $(date '+%F %T')" | tee "$LOG"
...
export MASTER_PORT=29695
export GLOO_SOCKET_IFNAME=eth0
export NCCL_SOCKET_IFNAME=eth0
export TP_SOCKET_IFNAME=eth0
export NCCL_SOCKET_FAMILY=AF_INET6
export TORCHINDUCTOR_CACHE_DIR=/tmp/ti_qwen25_math_7b_v33_efficient20
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  trainer.experiment_name=math-qwen25_math_7b-efficient-ttrl-v33-20step-val-answer-selection \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN25_MATH_7B_V33_EFFICIENT20_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V33_EFFICIENT20_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
  tail -160 "$METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V33_EFFICIENT20_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V33_EFFICIENT20_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V33_EFFICIENT20_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
exit "$status"
