...
EXP=sps_efficient_ttrl_qwen25_math_7b_50step_v53_low_budget_nonmajority_rescue_strict_n4
...
final_status_snapshot() {
  local status="$1"
  {
    echo "WORKER_QWEN25_MATH_7B_V53_LOW_BUDGET_NONMAJORITY_RESCUE_STRICT_N4_FINAL_STATUS ${status} $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V53_LOW_BUDGET_NONMAJORITY_RESCUE_STRICT_N4_START $(date '+%F %T')" | tee "$LOG"
...
ttrl_setup_cuda_env "q25v53" "$LOG"
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  ttrl.sps_low_budget_rescue_reward=True \
  ttrl.sps_low_budget_rescue_value=0.15 \
  ttrl.sps_low_budget_rescue_min_base=0.08 \
  ttrl.sps_low_budget_rescue_min_process=0.50 \
  ttrl.sps_low_budget_rescue_min_cluster_mass=0.03125 \
  ttrl.sps_low_budget_rescue_min_support=0.15 \
  ...
  trainer.experiment_name=math-qwen25_math_7b-efficient-ttrl-v53-low-budget-nonmajority-rescue-strict-n4 \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
set -e
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  ...
  echo "WORKER_QWEN25_MATH_7B_V53_LOW_BUDGET_NONMAJORITY_RESCUE_STRICT_N4_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN25_MATH_7B_V53_LOW_BUDGET_NONMAJORITY_RESCUE_STRICT_N4_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
...
PY
  ...
  echo "WORKER_QWEN25_MATH_7B_V53_LOW_BUDGET_NONMAJORITY_RESCUE_STRICT_N4_RAY_LOG_MISSING" | tee -a "$LOG"
fi

{
  echo "WORKER_QWEN25_MATH_7B_V53_LOW_BUDGET_NONMAJORITY_RESCUE_STRICT_N4_STATUS $status $(date '+%F %T')"
...
echo "WORKER_QWEN25_MATH_7B_V53_LOW_BUDGET_NONMAJORITY_RESCUE_STRICT_N4_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
trap - EXIT
final_status_snapshot "$status"
exit "$status"
