...
LOG=/opt/tiger/TTRL/verl/sps_format_anchor_coef02_sharpdiag_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_50step_v23.log
RAY_LOG_SNAPSHOT=/opt/tiger/TTRL/verl/sps_format_anchor_coef02_sharpdiag_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_v23_ray_taskrunner.log
METRICS_SNAPSHOT=/opt/tiger/TTRL/verl/sps_format_anchor_coef02_sharpdiag_beta20_floor015_clip05_power15_refbase_localfp32_qwen3_8b_8_v23_metrics.txt
RAY_DIR=/tmp/r23_qwen3_8b_format_anchor
SRC_MODEL=/opt/tiger/qwen3_8b
LOCAL_MODEL=/tmp/qwen3_8b_local_v21_answer_sharpen

echo "WORKER_QWEN3_8B_V23_START $(date '+%F %T')" | tee "$LOG"
...
export TORCHINDUCTOR_CACHE_DIR=/tmp/ti_qwen3_8b_v23_format_anchor
...
bash /opt/tiger/TTRL/verl/examples/ttrl/run_sps_rule_conf_weight_math_qwen3_4b_50step_8gpu.sh \
  ...
  ttrl.sps_format_reward_coef=0.2 \
  actor_rollout_ref.actor.use_kl_loss=True \
  trainer.experiment_name=math-qwen3_8b-sps-format-anchor-coef02-sharpdiag-beta20-floor015-clip05-power15-refbase-localfp32-v23-50step-8gpu \
  2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
...
if [ -n "$task_log" ] && [ -f "$task_log" ]; then
  cp "$task_log" "$RAY_LOG_SNAPSHOT" || true
  {
    ...
    grep -a -E "training/global_step|train/sps_format|train/sps/answer_|train/sps/majority_sharp|train/sps/train_weight|train/sps/weighted_label_confidence|train/ground_truth_reward|train/pass@32|train/majority_ratio|response_length/clip_ratio|val-core/MATH-TTT/acc|val-aux/MATH-TTT/acc|val-aux/MATH-TTT/format_score|Final validation metrics|global_step:50" "$task_log" || true
  } > "$METRICS_SNAPSHOT"
  echo "WORKER_QWEN3_8B_V23_RAY_LOG_SNAPSHOT $RAY_LOG_SNAPSHOT" | tee -a "$LOG"
  echo "WORKER_QWEN3_8B_V23_METRICS_SNAPSHOT $METRICS_SNAPSHOT" | tee -a "$LOG"
  tail -80 "$METRICS_SNAPSHOT" | tee -a "$LOG"
else
  echo "WORKER_QWEN3_8B_V23_RAY_LOG_MISSING" | tee -a "$LOG"
fi
echo "WORKER_QWEN3_8B_V23_EXIT status=${status} $(date '+%F %T')" | tee -a "$LOG"
exit "$status"
